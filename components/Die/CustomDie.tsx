 ...
 */
export const CustomDie = forwardRef<SVGSVGElement, CustomDieProps>(
  function CustomDie(
    {
      dieIdentifier = {
        left: 3,
    ...
    const downloadSVG = () => {
      ...
      if (svgElement) {
        // Clone the SVG and fix it for standalone use
        const clonedSvg = svgElement.cloneNode(true) as SVGSVGElement

        // Replace className with fill attribute for standalone SVG
        const pathElements = clonedSvg.querySelectorAll('path.st0')
        pathElements.forEach((path) => {
          path.setAttribute('fill', fill)
          path.removeAttribute('class')
        })

        // Remove the style tag since we're using direct fill attributes
...
CustomDie.displayName = 'CustomDie'
