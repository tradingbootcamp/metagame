    ...
    return (
      <TooltipPrimitive.Portal>
        <TooltipPrimitive.Content
          ...
          onMouseLeave={handleMouseLeave}
          className={cn(
            'z-50 w-fit max-w-[calc(100vw-2rem)] origin-(--radix-tooltip-content-transform-origin) animate-in rounded-md bg-bg-tooltip px-3 py-1.5 text-center text-xs text-balance text-text-tooltip fade-in-0 zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 sm:max-w-80',
            className,
          )}
          {...props}
        >
          {children}
          {showArrow && (
            <TooltipPrimitive.Arrow className="z-50 size-2.5 translate-y-[calc(-50%_-_2px)] rotate-45 rounded-[2px] bg-bg-tooltip fill-bg-tooltip" />
          )}
        </TooltipPrimitive.Content>
      </TooltipPrimitive.Portal>
    )
