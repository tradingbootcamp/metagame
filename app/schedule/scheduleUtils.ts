...
export const gCalLinkFromSession = (session: DbFullSession) => {
  ...
  const toCalendarString = (date: Date) => {
  ...
  if (session.start_time && session.end_time) {
    const startTimeString = toCalendarString(new Date(session.start_time))
    const endTimeString = toCalendarString(new Date(session.end_time))
